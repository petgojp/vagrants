...
class WP_Terms_List_Table extends WP_List_Table {
	...
	public function __construct( $args = array() ) {
		global $post_type, $category_taxonomy, $action, $tax;
		...
		$category_taxonomy  = $this->screen->taxonomy;

		if ( empty( $category_taxonomy ) )
			$category_taxonomy = 'post_tag';

		if ( ! taxonomy_exists( $category_taxonomy ) )
			wp_die( __( 'Invalid taxonomy' ) );

		$tax = get_taxonomy( $category_taxonomy );
	...
	public function single_row( $tag, $level = 0 ) {
		global $category_taxonomy;
 		$tag = sanitize_term( $tag, $category_taxonomy );
